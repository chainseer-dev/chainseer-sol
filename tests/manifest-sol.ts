...
import { prophecyWall } from "../target/types/prophecy_wall";
import { Keypair, PublicKey, LAMPORTS_PER_SOL } from "@solana/web3.js";
import { expect } from "chai";

describe("prophecy-sol", () => {
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);

  const program = anchor.workspace.prophecyWall as Program<prophecyWall>;
  ...
  describe("initialize_wall", () => {
    it("should initialize a new wall with dev wallet as owner", async () => {
      ...
      expect(wallAccount.devWallet.toString()).to.equal(
    ...
    it("should not allow non-dev wallet to initialize", async () => {
      ...
      const wallPDA = await getWallPDA(
  ...
  describe("post_message", () => {
    it("should allow user1 to post a message and transfer fee to dev wallet", async () => {
      ...
      const devBalanceDiff =
        (devBalanceAfter - devBalanceBefore) / LAMPORTS_PER_SOL;
    ...
    it("should fail when posting to non-existent wall", async () => {
      ...
      try {
        ...
        expect(error.toString()).to.include(
          "AnchorError caused by account: wall"
        );
      }
    });
    ...
    it("should fail when user has insufficient funds for message fee", async () => {
      ...
      const poorUserBalance = await provider.connection.getBalance(
        poorUser.publicKey
      );
      expect(poorUserBalance / LAMPORTS_PER_SOL).to.be.lessThan(
        MESSAGE_FEE + TX_FEE_BUFFER
      );
      ...
      try {
        ...
        expect(error.toString()).to.include(
          "Insufficient funds to pay message fee"
        );
      }
    });
    ...
    it("should fail when using wrong dev wallet", async () => {
      ...
      try {
        ...
        expect(error.toString()).to.include(
          "AnchorError caused by account: dev_wallet"
        );
      }
    });
    ...
    it("should verify exact fee transfer amount", async () => {
      ...
      const userBalanceDiff =
        (userBalanceBefore - userBalanceAfter) / LAMPORTS_PER_SOL;
